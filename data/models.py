...
class User(SqlAlchemyBase, UserMixin, SerializerMixin):
    ...
    seller_chat = orm.relationship(
        "Chats", back_populates='seller', foreign_keys="Chats.seller_id"
    )
    buyer_chat = orm.relationship(
        "Chats", back_populates='buyer', foreign_keys="Chats.buyer_id"
    )
...
class Goods(SqlAlchemyBase, SerializerMixin):
    ...
    is_active = sqlalchemy.Column(
        sqlalchemy.Boolean, nullable=True, default=True
    )

    category = orm.relationship(
        'Category', back_populates='goods'
    )
...
class Chats(SqlAlchemyBase, SerializerMixin):
    ...
    buyer_id = sqlalchemy.Column(
        sqlalchemy.Integer, sqlalchemy.ForeignKey("users.id"), nullable=False
    )
    good_id = sqlalchemy.Column(
        sqlalchemy.Integer, sqlalchemy.ForeignKey("goods.id"), nullable=False
    )
    created_date = sqlalchemy.Column(
        sqlalchemy.DateTime, default=datetime.datetime.now
    )

    seller = orm.relationship(
        'User', back_populates='seller_chat', foreign_keys=[seller_id]
    )
    buyer = orm.relationship(
        'User', back_populates='buyer_chat', foreign_keys=[buyer_id]
    )
    good = orm.relationship(
        'Goods', back_populates='chats'
    )
...
class Messages(SqlAlchemyBase, SerializerMixin):
    ...
    chat_id = sqlalchemy.Column(
        sqlalchemy.Integer, sqlalchemy.ForeignKey("chats.id"), nullable=False
    )
